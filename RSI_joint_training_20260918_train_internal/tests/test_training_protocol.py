...
from data_protocol import allocate,TRAIN_QUOTAS,META_PROBE_QUOTAS,exposure,scan
...
class Allocation(unittest.TestCase):
    def test_reproducible_quotas_round_disjoint_and_probe_containment(self):
        ...
        self.assertEqual(sum(len(x['tasks']) for x in first[:3]),3600)
        for value in first[:3]:
            self.assertEqual(Counter(x['source'] for x in value['tasks']),TRAIN_QUOTAS)
            probes=[x for x in value['tasks'] if x['meta_probe']]
            self.assertEqual(Counter(x['source'] for x in probes),META_PROBE_QUOTAS)
            assert_disjoint([value,make_manifest('train_evolution',value['round_id'],probes)])
...
class Boundaries(unittest.TestCase):
    def setUp(self):
        ...
        self.row=dict(task_id=t['task_id'],source_role='train_evolution',purpose='evolution_train',round_id=1,branch_id='parent',
    ...
    def test_sft_accepts_M_and_rejects_external_wrong_round_harness_parent(self):
        ...
        for change in ({'source_role':'independent_validation'},{'source_role':'final_test'},
                       {'purpose':'report_only'},{'round_id':2},{'harness_hash':'old'},{'parent_policy_hash':'old'}):
...
class NativeRoundLoop(unittest.TestCase):
    ...
    def run_fixture(self,root,actions,interrupt=False):
        ...
        class Executor:
            def execute(executor,state,directory):
                if interrupt and not interrupted[0] and store.round_id==1 and 'MODEL' in directory.parts and store.mode=='probe':
                    interrupted[0]=True
                ...
                (directory/'train_trajectories.jsonl').write_text(json.dumps(row)+'\n' if store.mode=='training' else '')
                # Every candidate improves in this mock, so exactly one candidate per round.
                score=1 if 'candidates' in directory.parts and store.mode=='probe' else 0
                row['verification']['success']=bool(score);row['terminal_reward']=score
                (directory/'probe_trajectories.jsonl').write_text(json.dumps(row)+'\n')
                return EvaluationResult({'macro_success':score,'probe_identity':store.probes['manifest_hash'],'domains':{'tool_use':{'count':1,'successes':score}}},[row])
        ...
        class Provider:
            ...
            def complete(self,prompt,schema,**kwargs):
                ...
                self_outer.assertIsNone(kwargs['operation_input']['trusted_facts']['feedback_protocol']['current_round_delta'])
        ...
        class Meta:
            ...
            def learn_from_experience(self,*args):raise AssertionError('No post-evaluation fourth or second Meta update')
        class Updater:
        ...
        self.assertEqual(sum(e[0]=='trainer' for e in events),actions.count('MODEL'))
        self.assertEqual(sum(e[0]=='meta_update' for e in events),3)
        for r in (1,2,3):
            self.assertLess(events.index(('meta_update',r)),next(i for i,e in enumerate(events) if e[:2]==('decision',r)))
...
class NativeRolloutReuse(unittest.TestCase):
    def test_M_to_B_reuse_requires_complete_generation_state(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                with patch('sia.task_meta.seed.load_seed',return_value={'schema_version':1}),patch('sia.task_meta.seed.run_seed',side_effect=run_seed):
                    ...
                    first=durable.execute(state,before);self.assertEqual(len(calls),3)
                    self.assertEqual(len(first.trajectories),3)
                    train=root/'collect';protocol.bind_execution(state,train,'training')
                    collected=durable.execute(state,train);self.assertEqual(len(calls),6)
                    self.assertEqual(len(collected.trajectories),6);self.assertEqual(collected.cost['physical_rollout_attempts'],3)
                    self.assertTrue(all(r['purpose']=='evolution_train' and not r['notes'] for r in collected.trajectories))
                    protocol.meta.version=1;changed=root/'changed';protocol.bind_execution(state,changed)
                    durable.execute(state,changed);self.assertEqual(len(calls),9)
