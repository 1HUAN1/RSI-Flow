...
from evolution_protocol import DOMAINS, assert_disjoint, file_hash, fingerprint, freeze, make_manifest, read

TRAIN_QUOTAS={'envscaler':400,'deepcoder_taco':400,'nq_open':100,'hotpotqa':150,'2wiki':150}
META_PROBE_QUOTAS={'envscaler':80,'deepcoder_taco':80,'nq_open':20,'hotpotqa':30,'2wiki':30}
...
def scan(sources, grouping):
    ...
    return items,dict(files=stats,duplicate_ids=duplicates,valid_unique_ids=len(items),group_count=len(groups),
...
def exposure(row, ledger):
    ...
    for event in ledger.get('events',[]):
        ...
        if (event.get('all_tasks') or row['task_id'] in event.get('task_ids',[]) or
            row['native_id'] in event.get('task_ids',[]) or row['group_id'] in event.get('group_ids',[]) or
...
def allocate(items, config, ledger):
    ...
    for r,rows in enumerate(bins,1):
        ...
        probe=[]
        for source,count in config['meta_probe_quotas'].items():
            probe+=balanced_sample([x for x in selected if x['source']==source],count,seed+r+1000)
        probe_ids={x['task_id'] for x in probe}
        tasks=[dict(x,role='train_evolution',purpose='evolution_train',round_id=r,meta_probe=x['task_id'] in probe_ids) for x in selected]
        manifests.append(make_manifest('train_evolution',r,tasks,split_seed=seed,meta_probe_tasks=len(probe)))
    ...
    for source,limit in config['validation_limits'].items():
        ...
        selected=sorted(candidates,key=lambda r:r['task_id']) if limit is None else balanced_sample(candidates,min(limit,len(candidates)),seed+9000)
    ...
    for source,rows in by_source.items():
        ...
        with target.open('xb') as out:
            for row in sorted(rows,key=lambda x:x['task_id']):
                ...
                if text_hash(public_text(raw,source))!=row['content_hash']:raise ValueError('Source record changed during packing')
                line=(json.dumps(raw,ensure_ascii=False)+'\n').encode();offset=out.tell();out.write(line)
                tasks.append({**{k:v for k,v in row.items() if k not in ('path','offset','length','kind','feedback_priority')},
...
def prepare_protocol(config, evaluation, output, final_root):
    ...
    for value in manifests:
        ...
        if role=='train_evolution':
            probe=[t for t in packed['tasks'] if t['meta_probe']]
            freeze(directory/'meta_probe.json',make_manifest('train_evolution',r,probe,selection='meta_probe',parent_manifest_hash=packed['manifest_hash']))
    ...
    report=dict(status='prepared_not_executed',allocated_tasks=3600,executed_unique_tasks=0,rollout_attempts=0,
    ...
    try:
        ...
        for value in rounds:
            for row in value['tasks']:
                ...
                seq[row['domain']]+=1
                if row['meta_probe']:conn.execute('INSERT INTO probe VALUES(?)',(row['task_id'],))
        conn.commit()
    finally: conn.close()
...
def main():
    ...
    p.add_argument('--output',required=True);p.add_argument('--final-root',required=True)
    a=p.parse_args();print(json.dumps(prepare_protocol(read(a.config),read(a.evaluation_config),a.output,a.final_root),ensure_ascii=False,indent=2))
