...
from evolution_protocol import manifest, file_hash, freeze


def select_specs(settings, path, role, out):
...
def evaluation_budget(path,role):
    ...
    return dict(role=role,manifest_hash=value['manifest_hash'],tasks=len(value['tasks']),
        by_source=dict(Counter(t['source'] for t in value['tasks'])),checkpoints=['A0','A1','A2','A3'] if role=='independent_validation' else ['A0','A3'],
