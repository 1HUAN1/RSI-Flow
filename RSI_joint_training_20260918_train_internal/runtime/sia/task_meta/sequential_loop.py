...
def run_sequential_task_meta(run_dir, task_state, meta_state, executor, meta_agent, updaters,
    ...
    for number in range(max_generations):
        ...
        for index in range(3):
            ...
            if observation_path.exists():
                ...
                if base_observation is None:
                    ...
                    if round_protocol and TaskUpdateAction.MODEL in updaters:
                        ...
                        capabilities['MODEL']['reason'] = 'Collect current B_r only if selected; verified successes and fixed minimum required'
                    ...
                    base_observation.budget.update(policy=POLICY, candidate_limit=3, max_generations=max_generations,
                ...
                observation.budget.update(candidate_index=index, attempted_components=sorted(tried), immutable_task_policy=(CONTRACT + ' Meta was updated before this decision from current M_r. Round-end experience is only for the next round. External validation is never an optimization input.' if round_protocol else CONTRACT))
                for action in tried:
            ...
            if attempt['status'] not in {'evaluated', 'unavailable'}:
                try:
                    ...
                    attempt.update(status='evaluated', candidate_executed=True, candidate_state=asdict(candidate),
                        ...
                        paired_outcome=outcome, gain=outcome['success_delta'], positive_gain=accepted)
                    save_json(attempt_path, attempt)
            ...
            print('[candidate-result] ' + json.dumps({'round': number, 'component': action,
                ...
                'gain': attempt.get('gain'), 'accepted': attempt.get('positive_gain', False),
                'generating_G': attempt['generating_G']['version']}), flush=True)
            if attempt.get('positive_gain'):
        ...
        deployment = {'status': 'deployed' if chosen else 'parent_retained', 'round': number,
            ...
            'rule': POLICY, 'gain': chosen['gain'] if chosen else 0.0}
    ...
    final = {'status': 'completed' if len(rounds) == max_generations else 'budget_exhausted',
        ...
        'final_consolidation_status': ('archived_no_fourth_update' if round_protocol else 'last_round_memory_update_completed') if len(rounds) == max_generations else 'pending'}
