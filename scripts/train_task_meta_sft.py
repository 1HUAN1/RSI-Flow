...
def read_positive_rows(request_dir: Path, request: dict) -> list[dict]:
    ...
    if type(max_samples) is not int or not 1 <= max_samples <= 100000:
        raise ValueError("training.max_samples must be an integer in [1, 100000]")
    rows = []
...
def resolve_training_options(request, args):
    ...
    defaults = {"max_steps": 8 if legacy else None, "learning_rate": 1e-3 if legacy else 2e-5,
    ...
    bounds = {"max_steps": (1, 10000), "max_length": (32, 32768), "lora_rank": (1, 256),
              "batch_size": (1, 32), "gradient_accumulation_steps": (1, 256),
              "save_steps": (0, 10000), "max_samples": (1, 100000), "seed": (0, 2**32 - 1)}
...
def train(args) -> dict:
    ...
    save_json(request_dir / "resolved_training_config.json", options)
    ...
    save_json(request_dir / "sft_selection.json", selection_report)
    ...
    model = get_peft_model(model, LoraConfig(task_type="CAUSAL_LM", r=options["lora_rank"],
                                           lora_alpha=2 * options["lora_rank"], lora_dropout=0.0,
    ...
    training_args = TrainingArguments(
        output_dir=str(request_dir / "trainer_state"), max_steps=options["max_steps"],
    ...
    if trainer.state.global_step != options["max_steps"] or not math.isfinite(result.training_loss):
        raise RuntimeError("Trainer did not complete the requested finite SFT steps")
    delta_squared = 0.0
    ...
    manifest = register_checkpoint(args.base_url, checkpoint)
    report["serving_status"] = "ready"
    save_json(request_dir / "training_metrics.json", report)
    ...
    save_json(request_dir / "checkpoint.json", manifest)
    return report
