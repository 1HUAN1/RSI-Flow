...
def prepare(config, runtime):
    if config.get('training_schedule') == 'round_disjoint':
        from data_protocol import prepare_protocol
        from common import ROOT
        return prepare_protocol(config,read(ROOT/config['validation_config']),Path(runtime)/'data/rounds',config['validation_vault'])
