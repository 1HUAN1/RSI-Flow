...
from .task_harness import harness_identity
...
def _generate_report_predictions(config, frozen_path, specs_path, output_dir):
    ...
    fixed_harness_identity = None
    if seed_spec['schema_version'] == 2:
        fixed_harness_identity = harness_identity(state.harness_path)
        if frozen.get('task_harness_identity') != fixed_harness_identity:
            raise ValueError('Frozen Task Harness runtime identity is absent or changed')
    ...
    if fixed_harness_identity is not None:
                ...
                for row in iterator:
                    ...
                    try:
                        ...
                        result = run_seed(seed_spec, model, environment, json.dumps(public), artifacts, seed,
                                          artifact_sources=artifact_sources if seed_spec['schema_version'] == 2 else None)
