...
class TaskContextBudgetExceeded(RuntimeError):
    """The local server rejected the input before model generation."""
    ...
    def __call__(self, messages, *, tools=None, seed, max_tokens, temperature):
        ...
        try:
            ...
            try:
                ...
                detail = None
            if exc.code == 400 and detail == 'Request exceeds the configured Task context budget':
                raise TaskContextBudgetExceeded('Task context budget exhausted; no generation dispatched') from exc
            raise TaskInfrastructureError(f'Local checkpoint request failed: HTTP {exc.code}') from exc
