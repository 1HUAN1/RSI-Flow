...
class ResponsesTransport:
    ...
    def validate_body(self, body):
        ...
        if self.config.provider == "autodl" and self.config.model in {"DeepSeek-V4-Flash", "gpt-5.6-sol"} and self.config.g_output_delivery == "candidate_file_all" and body.get("tools"):
            text_options = dict(body.get("text") or {})
            text_options.pop("format", None)
            if text_options:
                body["text"] = text_options
