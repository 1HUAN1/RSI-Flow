...
def run_harness(spec, model_callable, environment, task_prompt, artifacts_text="", seed=42, *, artifact_sources=None):
    ...
    def complete(operation, messages, *, role=None):
        ...
        try:
            ...
            for key in ("chat_template_kwargs", "finish_reason", "request_id", "model_ref_requested", "model_ref_response", "raw_generation"):
                if key in response:
            ...
            record.update(status='rejected', error_type=type(exc).__name__, usage_complete=True, unknown_usage_calls=0)
            event('model_context_budget_exhausted', call_id=record['call_id'], generation_dispatched=False)
    ...
    def parse_action(assistant):
        from .action_contract import normalize_action
        return normalize_action(assistant, tools, parts["tools"]["action"]["max_tools_per_step"])
    ...
    try:
        while True:
            ...
            event("transition", primitive=kind, enabled=enabled,
                  component_type={"planning": "Planning", "memory": "Memory", "act": "Reasoning",
                    "tools": "Tools", "inspect": "Verification", "commit": "Context",
                    "finalize": "Verification", "role": "Reasoning", "branch": "Planning", "stop": "Verification"}.get(kind, "Context"))
            if enabled:
                if kind == "stop":
                    break
