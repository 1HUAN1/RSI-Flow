...
class SubmissionEvaluator:
    ...
    def execute(self, state, directory):
        ...
        for name, old in source.items():
            ...
            binding = {'state_hash': task_hash(state), 'baseline_sha256': value_hash(old), 'edited': name in self.targets}
            if receipt.exists():
                ...
                row.update(state_hash=task_hash(state), model_calls=[], transport_calls=[], sft_conversations=[],
                    ...
                    baseline_row_sha256=value_hash(old), events=[])
                if name in self.targets:
                    ...
                    try:
                        ...
                        row.update(final_answer=value['final_answer'], model_answer=None,
                            ...
                            error_type=score.error_type, verifier_details=score.details,
        ...
        costs = {'model_calls': 0, 'input_tokens': 0, 'output_tokens': 0, 'unknown_usage_calls': 0,
                 ...
                 'wall_time_seconds': sum(v['wall_time_seconds'] for vs in rows.values() for v in vs),
