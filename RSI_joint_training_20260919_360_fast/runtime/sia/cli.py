...
def _add_run_args(parser: argparse.ArgumentParser, env_config: Config) -> None:
    ...
    parser.add_argument(
        "--evolution-mode",
        choices=["sia", "task-meta"],
        default="sia",
        help="Original SIA loop or Task/Meta co-evolution (default: sia)",
    )
    parser.add_argument(
        "--task-meta-config",
        default=None,
        help="JSON configuration for task-meta batch, rollouts, metric, budget and optional trainer",
    )
    parser.add_argument(
        "--max_gen",
        type=int,
