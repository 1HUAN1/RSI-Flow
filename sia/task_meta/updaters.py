...
from sia.task_meta.harness import EDITABLE_FUNCTIONS, validate_harness_update
from sia.task_meta.storage import artifact_manifest, checkpoint_manifest, clone_task, digest, save_json
...
def updater_capabilities(task_state, evaluation, trainer_configured=True, *, sft_profile="legacy_gpqa"):
    ...
    capabilities = {
        "HARNESS": {"available": True, "operations": [{"operation": "replace_hook", "targets": sorted(EDITABLE_FUNCTIONS)}],
                    "constraints": ["Only pure hook bodies may change; signatures and protected I/O stay fixed",
                                    "No direct asset content, checkpoint, evaluator, dataset or credential changes",
                                    "harness_config keys: max_attempts (1..3), use_artifacts, generate_artifacts, "
                                    "artifact_char_limit (0..12000), retry_on_parse_error, retry_on_api_error",
                                    "harness_config must directly return a literal dictionary with exactly those six keys",
                                    "Complex external tools and retrieval workflows are not implemented"]},
    ...
    if sft_profile == "multidomain":
        from sia.task_meta.seed import seed_capabilities
        capabilities["HARNESS"] = seed_capabilities(task_state.harness_path)
    return capabilities
...
def validate_decision_targets(decision, action):
    ...
    for change in requested:
        if action == TaskUpdateAction.HARNESS:
            valid = change.operation == "replace_hook" and change.target in EDITABLE_FUNCTIONS
...
def _hook_bodies(code):
    return {node.name: ast.dump(ast.Module(body=node.body, type_ignores=[]), include_attributes=False)
            for node in ast.parse(code).body if isinstance(node, ast.FunctionDef) and node.name in EDITABLE_FUNCTIONS}


class HarnessUpdater:
    def __init__(self, client, task_files, provider):
        self.client, self.task_files, self.provider = client, task_files, provider

    def apply(self, task_state, decision, context):
        started = time.monotonic()
        requested = validate_decision_targets(decision, TaskUpdateAction.HARNESS)
        old_code = Path(task_state.harness_path).read_text(encoding="utf-8")
        prompt = build_feedback_prompt(
            current_gen=task_state.generation, max_gen=context.generation + 1, task_files=self.task_files,
            agent_py=old_code, task=self.task_files.task_md,
            execution_status=json.dumps(context.evaluation.performance),
            execution_section=json.dumps(context.observation.success_examples + context.observation.failure_examples, ensure_ascii=False),
            run_dir=str(context.directory.parent), next_gen_dir=str(context.directory), previous_gens="See decision observation",
            task_model=task_state.model_ref, provider=self.provider, focus="harness",
        )
        prompt += ("\nTASK-META OUTPUT CONTRACT (replaces file-tool instructions above): Return the full replacement "
                   "target_agent.py in code and a concise implementation summary. Change exactly the requested hook "
                   "bodies. Preserve every signature, import, top-level statement and protected runtime/I/O body. "
                   "Do not alter model/checkpoint bindings, task data, evaluator, credentials, logs or any asset file. "
                   "The hook may change how current assets are used and new notes produced; it cannot restore or edit "
                   "asset contents directly. Hooks are pure: local variables, ordinary string/list/dict operations, "
                   "safe builtins, json and re only. No files, network, globals, imports, nested definitions or mutation "
                   "of arguments. Use only the requested targets, even if another change would be helpful. "
                   "Requested natural-language effects remain hypotheses until actually observed.\nCapabilities:\n"
                   + json.dumps(updater_capabilities(task_state, context.evaluation)["HARNESS"], ensure_ascii=False)
                   + "\nDecision:\n" + decision.model_dump_json())
        def candidate(value):
            value = ProgramUpdate.model_validate(value)
            try:
                validate_harness_update(old_code, value.code)
                old_hooks, new_hooks = _hook_bodies(old_code), _hook_bodies(value.code)
                changed = {name for name in old_hooks if old_hooks[name] != new_hooks[name]}
                targets = {change.target for change in requested}
                if changed != targets:
                    raise ValueError(f"Requested hooks {sorted(targets)}; actually changed {sorted(changed)}. "
                                     f"Unapplied: {sorted(targets - changed)}; undeclared: {sorted(changed - targets)}")
            except (ValueError, SyntaxError) as exc:
                raise DecisionConstraintError(str(exc)) from exc
            return old_hooks, new_hooks

        from sia.task_meta.meta import evolution_kwargs
        if getattr(self.client, "supports_evolution", False):
            prompt = "Return replacement Task code changing only the declared pure hook bodies. Preserve interfaces and the protected runtime."
        output = self.client.complete(prompt, ProgramUpdate, meta_state=context.meta_state,
            operation="harness_patch", decision_id=decision.decision_id,
            **evolution_kwargs(self.client, context, task_state, decision, {"target_agent.py": old_code}, candidate))
        old_hooks, new_hooks = candidate(output)
        new_state = clone_task(task_state, context.generation, context.directory)
        Path(new_state.harness_path).write_text(output.code, encoding="utf-8")
        applied = [{"id": change.id, "operation": change.operation, "target": change.target,
                    "before_sha256": _text_hash(old_hooks[change.target]),
                    "after_sha256": _text_hash(new_hooks[change.target]), "semantic_status": "unverified"}
                   for change in requested]
        files = [{"path": "target_agent.py", "before_sha256": _text_hash(old_code),
                  "after_sha256": digest(Path(new_state.harness_path))}]
        checks = [{"name": "protected_runtime_and_pure_hooks", "passed": True},
                  {"name": "all_and_only_requested_hook_bodies_changed", "passed": True}]
        return new_state, TaskUpdate(TaskUpdateAction.HARNESS, output.summary,
                                     {"harness_path": new_state.harness_path}, _cost(started),
                                     **_audit(decision, applied, files, checks))
...
class ModelUpdater:
    ...
    def _apply(self, task_state, decision, context):
        ...
        from sia.task_meta.sft import select_positive_rows
        positives = select_positive_rows(context.evaluation.trajectories, profile=self.sft_profile)
        if not positives:
        ...
        request_dir = Path(task_state.harness_path).parent / "model_update"
