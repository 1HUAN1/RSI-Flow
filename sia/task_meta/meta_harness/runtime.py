...
def execute(bundle, operation, operation_input, schema, invoke_stage, audit_dir, *,
    ...
    if type(budget["wall_time_seconds"]) not in (int, float) or not 0 < budget["wall_time_seconds"] <= 3600:
    ...
    state = {"operation": operation, "candidate": None, "candidate_valid": False, "checks": {},
    ...
    def guard():
        ...
        if _hash(operation_input) != source_snapshot_hash or _hash(envelope) != source_hash:
    ...
    def model_step(step, stage_id, *, repair=False):
        ...
        inline_payload = copy.deepcopy(payload)
        for index, dependency in enumerate(inline_payload["dependencies"]):
        ...
        prompt = ("Current executable G mechanism instructions:\n" + instruction
                  ...
                  "Only runtime source contents are referenced instead of duplicated; read them with Python from the stated JSON paths if needed. "
    ...
    save()
    try:
        for step in policy["workflows"][operation]:
            guard()
            ...
            if kind == "evidence":
                ...
                paths = step.get("paths", list(current_files)[:16])
                reads = []
                for path in paths:
                    ...
                    if kind == "inspect_targets":
                        ...
                        for change in envelope.get("decision", {}).get("requested_changes", []):
                            ...
                            observed = lookup(document, target, _MISSING) if isinstance(target, str) else _MISSING
