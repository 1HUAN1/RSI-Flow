...
SCHEMA_FILES = {"meta-bundle-v1": LEGACY_EDITABLE_FILES, "meta-bundle-v2": EDITABLE_FILES}
...
def validate_files(files):
    ...
    context = ContextPolicy.model_validate(strict_json(files["context.json"]), strict=schema == "meta-bundle-v2")
    if context.selection not in {"head", "tail", "head_and_tail"}:
        raise ValueError("Unsupported context selection")
    workflow = WorkflowPolicy.model_validate(strict_json(files["workflow.json"]), strict=schema == "meta-bundle-v2")
...
class MetaHarnessBundle:
    ...
    def verify(self):
        ...
        if self.schema_version == "meta-bundle-v2":
    ...
    def render(self, evidence: str, response_contract: str, *, preselected=False):
        ...
        files = self.read_files()
        context = ContextPolicy.model_validate(strict_json(files["context.json"]), strict=self.schema_version == "meta-bundle-v2")
        workflow = WorkflowPolicy.model_validate(strict_json(files["workflow.json"]), strict=self.schema_version == "meta-bundle-v2")
...
class MetaHarnessStore:
    ...
    def initialize(self, seed_path: Path, source_commit: str | None = None, binary_sha256: str | None = None):
        ...
        if names not in (LEGACY_EDITABLE_FILES, EDITABLE_FILES):
