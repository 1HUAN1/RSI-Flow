...
def sdk_bridge():
    ...
    class Completions:
        def create(self,**kwargs):
            ...
            raw=json.dumps(plain(kwargs)).encode()+b'\n'
            if len(raw)>8_000_000:raise ValueError('RPC request too large')
            with lock:
                outgoing.write(raw);outgoing.flush();line=incoming.readline(8_000_001)
            if not line or len(line)>8_000_000:raise RuntimeError('RPC transport failed')
...
def main():
    ...
    if benchmark=='bfcl_v3':
        ...
        if phase=='generate':sys.argv+=['--num-threads','1','--temperature','0','--user-model',spec['user_model']]
