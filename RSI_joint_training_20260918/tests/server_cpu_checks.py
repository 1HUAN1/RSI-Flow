...
def main():
    ...
    try:
        ...
        for spec in config['datasets']:
            ...
            for relative in spec['paths']:
                with (Path(config['dataset_root'])/relative).open('rb') as stream:
                    count+=sum(bool(line.strip()) for line in stream)
            counts[spec['name']]=count
            if count!=spec['count']:raise ValueError('Dataset count mismatch: '+spec['name'])
    ...
    try:
        from prepare_data import audit_validation_overlap
        audit=audit_validation_overlap(read(ROOT/'configs/train.json'),read(ROOT/'configs/validation.json'))
        results.append({'check':'data_overlap','status':'passed',**audit})
    ...
    output=ROOT/'preflight/isolation'
    output.mkdir(parents=True,exist_ok=True)
    secret=ROOT/'preflight/outside_sandbox_fixture.txt';secret.write_text('test_override_only')
...
''')
    ...
    write(ROOT/'preflight/server_cpu_checks.json',report)
