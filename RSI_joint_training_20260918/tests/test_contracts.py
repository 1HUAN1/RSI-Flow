...
for name in ('runtime_extensions.py','round_validation.py'):
...
def tearDownModule():TEMP.cleanup()
...
class DataAndEpoch(unittest.TestCase):
    def test_counts_and_separate_configs(self):
        ...
        self.assertEqual(sum(d['count'] for d in train['datasets']),346219)
        self.assertEqual(train['expected_total'],346219)
        ...
        self.assertTrue(all('test/' not in p for d in train['datasets'] for p in d['paths']))
