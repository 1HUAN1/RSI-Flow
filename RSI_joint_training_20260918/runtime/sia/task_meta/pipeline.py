...
class PipelineConfig(BaseModel):
    ...
    task_update_policy: Literal['always_apply', 'sequential_positive_gain'] = 'always_apply'
    ...
    training_schedule: Literal['stream', 'fixed_subset', 'full_cohort'] = 'stream'
    ...
    def checked(self):
        ...
        if self.artifact_evaluation == 'direct_submission' and self.task_update_policy != 'sequential_positive_gain':
            raise ValueError('Direct submissions require frozen same-parent sequential comparisons')
...
def _run(config, run_dir, *, resume=False):
    ...
    if config.task_update_policy == 'sequential_positive_gain':
        protocol['selection_rule'] = 'sequential_same_parent_first_positive_v1'
    ...
    if continuation:
        ...
        for key in ('config', 'model_identity', 'data_hash', 'corpus', 'seed_hash', 'data_scope'):
            ...
            if key == 'config':
                ...
                if current_value.get('task_update_policy') != previous_value.get('task_update_policy', 'always_apply'):
                    if permitted.get('task_update_policy') != {'before': previous_value.get('task_update_policy', 'always_apply'), 'after': current_value['task_update_policy']}:
                        raise ValueError('Unregistered Task candidate acceptance policy change')
                if 'task_update_policy' in previous_value:
                    current_value['task_update_policy'] = previous_value['task_update_policy']
                else:
                    current_value.pop('task_update_policy')
    ...
    evaluated_prefix = None
    if continuation and continuation.get('evaluated_prefix'):
        from sia.task_meta.types import ImprovementExperience
        prefix = continuation['evaluated_prefix']
        source = Path(continuation['source_run'])
        for relative, expected in prefix['file_hashes'].items():
            if not Path(relative).is_relative_to('gen_' + relative.split('/')[0].split('_')[-1]):
                raise ValueError('Invalid prefix path')
            for base in (source, run_dir):
                path = (base / relative).resolve()
                if not path.is_relative_to(base.resolve()) or digest(path) != expected:
                    raise ValueError('Evaluated prefix evidence changed: ' + relative)
        generation = prefix['generation']
        initial_task = load_task(json.loads((source / f'gen_{generation - 1}/intervention_receipt.json').read_text())['successor'])
        evaluated_prefix = {
            'scores': [{'generation': i, **json.loads((source / f'gen_{i}/results.json').read_text())} for i in range(generation)],
            'costs': [{'generation': i, **json.loads((source / f'gen_{i}/cost.json').read_text())} for i in range(generation)],
            'history': [ImprovementExperience(**json.loads((source / f'gen_{i}/improvement_experience.json').read_text())) for i in range(1, generation + 1)],
            'pending_experience': prefix['pending_experience'],
        }
    ...
    if continuation and continuation.get('evaluated_prefix') and (continuation.get('accepted_meta_state_relative') or not continuation['evaluated_prefix']['pending_experience']):
        relative_meta = continuation.get('accepted_meta_state_relative', 'gen_3/meta_state_after.json')
        prefix_generation = continuation['evaluated_prefix']['generation']
        if relative_meta not in {f'gen_{prefix_generation}/meta_state_before.json', f'gen_{prefix_generation - 1}/meta_state_after.json'}:
            raise ValueError('Accepted Meta state must match the evaluated prefix boundary')
        prior_meta = Path(continuation['source_run']) / relative_meta
        if digest(prior_meta) != continuation['accepted_meta_state_sha256']:
            raise ValueError('Accepted source Meta state changed')
        initial_meta = MetaAgentState(**json.loads(prior_meta.read_text()))
    ...
    def accept(meta_state, update):
        ...
        return meta_state
    try:
        windows = (config.max_generations if config.training_schedule in {'fixed_subset','full_cohort'} else
                   max(math.ceil(v['evolve_train'] / config.window_quotas[d]) for d, v in store.counts().items()))
        runner = run_task_meta
        runner_options = {}
        if config.task_update_policy == 'sequential_positive_gain':
        ...
        final = runner(run_dir, initial_task, initial_meta, executor, meta, updaters,
            ...
            resume=resume, meta_update_handler=accept, evaluated_prefix=evaluated_prefix, **runner_options)
...
def freeze(run_dir):
    ...
    sequential = final.get('task_update_policy') == 'sequential_same_parent_first_positive_v1'
    best = (final['performance_history'][-1] if sequential else
            max(final['performance_history'], key=lambda r: (r['macro_success'], -r['generation'])))
    state = (load_task(final['task_state']) if sequential else
             load_task(json.loads((root / f'gen_{best["generation"]}/evaluated_state.json').read_text())))
    ...
    frozen = {'task_state': asdict(state), 'state_hash': task_hash(state), 'protocol_hash': protocol['hash'],
              ...
              'selection': {'rule': final['task_update_policy'] if sequential else 'max_fixed_probe_macro_success_earliest_tie', 'generation': best['generation'],
    ...
    save_json(root / 'last_evaluated.json', asdict(state) if sequential else json.loads((root / f'gen_{final["performance_history"][-1]["generation"]}/evaluated_state.json').read_text()))
