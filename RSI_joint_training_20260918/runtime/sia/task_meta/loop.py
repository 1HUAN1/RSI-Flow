    ...
    *, test_decision_override=None, resume=False, meta_update_handler=None, evaluated_prefix=None,
    ...
    try:
        while True:
            ...
            current = result.performance['domains']['tool_use']
            prior = scores[-2]['domains']['tool_use'] if len(scores) > 1 else None
            print('[paired-probe] ' + json.dumps({'generation': task.generation, 'before': prior, 'after': current,
                'delta_success_rate': current['success_rate'] - prior['success_rate'] if prior else None,
                'delta_partial_score': current['mean_native_partial_score'] - prior['mean_native_partial_score'] if prior else None,
                'train_current_window_only': result.performance.get('training_rewards', {}).get('domains', {}).get('tool_use')}, ensure_ascii=False), flush=True)
            for label in ('train', 'probe'):
                file = gen_dir / (label + '_trajectories.jsonl')
                if file.exists():
                    for line in file.read_text().splitlines():
                        row = json.loads(line)
                        print('[task-result] ' + json.dumps({'generation': task.generation, 'split': label,
                            'task_id': row['task_id'], 'success': row['verification'].get('success'),
                            'metrics': row.get('metrics'), 'error_type': row.get('error_type'),
                            'model_calls': row.get('model_call_count')}, ensure_ascii=False), flush=True)
            ...
            elif evaluated_prefix and evaluated_prefix.get('pending_experience') and task.generation == task_state.generation:
                experience = history[-1]
                if experience.experience_id != evaluated_prefix['pending_experience'] or experience.generation + 1 != task.generation:
                    raise ValueError('Pending experience does not match evaluated prefix')
                learned = meta_agent.learn_from_experience(meta, experience, history, task_in)
                meta = _accept_meta_update(run_dir, meta, learned, [experience], gen_dir / "meta_self_update.json", "learn_from_experience", meta_update_handler, resume)
            ...
            for attempt in range(3):
                ...
                try:
                    ...
                    if budget.stop_after(task.generation + 1) and not replay_intervention:
                        ...
                        break
                    from sia.task_meta.meta_harness.five_stage import freeze_expectation
                    freeze_expectation(run_dir, task_out, meta, decision, observation)
            ...
            print('[task-intervention] ' + json.dumps({'generation_before': task.generation,
                'generation_after': successor.generation, 'G_version': meta.version, 'action': decision.action.value,
                'diagnosis': decision.diagnosis, 'proposed_change': decision.proposed_change,
                'expected_effect': decision.expected_effect, 'summary': update.summary,
                'requested': update.requested_changes, 'applied': update.applied_changes, 'diff': intervention}, ensure_ascii=False), flush=True)
            if decision.action.value == 'HARNESS':
                before_doc = json.loads(Path(task_out.harness_path).read_text())
                after_doc = json.loads(Path(successor.harness_path).read_text())
                for change in decision.requested_changes:
                    before_value, after_value = before_doc, after_doc
                    for part in change.target.split('.'):
                        before_value, after_value = before_value[part], after_value[part]
                    print('[harness-change] ' + json.dumps({'target': change.target,
                        'before': before_value, 'after': after_value}, ensure_ascii=False), flush=True)
