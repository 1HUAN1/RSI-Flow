...
def build_meta_prompt(
    ...
    request_timeout_seconds = provider.request_timeout_seconds if provider is not None else 60
...
9. Any network/API call made by target_agent.py MUST use a finite request timeout of {request_timeout_seconds} seconds or less, and retry no more than 2 times before recording an error for that sample. Do not let one stalled model response block the entire run indefinitely.
...
(do NOT use the anthropic or google SDK):
    ...
    client = OpenAI(
        ...
        timeout={float(provider.request_timeout_seconds)},
    )
