...
def main():
    ...
    directory = contained_path(ROOT / 'runs', args.request_dir)
    ...
    with exclusive_lock(ROOT / 'local_baseline/four_gpu_phase.lock'):
        stop_services()
        ...
        try:
            with contextlib.ExitStack() as leases:
                ...
                command = [config.trainer_python, '-m', 'torch.distributed.run', '--standalone',
                           ...
                           '--runs-dir', str(ROOT / 'runs'), '--base-url', config.task_base_url, '--defer-serving']
