...
def main():
    ...
    if args.runtime_source:
        config['runtime_source']=args.runtime_source
    ...
    try:
        from sia.task_meta.harnessforge_manifest import PINNED_HARNESSFORGE_COMMIT
        ...
        results.append({'check':'in_tree_harnessforge_runtime','status':'passed',
                        'runtime':str(source),'upstream_commit':PINNED_HARNESSFORGE_COMMIT,
                        'components':[HarnessForgeProductionUpdater.__name__,run_harnessforge.__name__]})
    except Exception as exc:
        results.append({'check':'in_tree_harnessforge_runtime','status':'failed','reason':str(exc)})
    try:
        counts={}
