...
class FinalProtocol(unittest.TestCase):
    ...
    def test_real_dry_run_counts_include_optional_A0_only_when_enabled(self):
        ...
        config=native.read(native.BUNDLE/'configs/train.json');config['validation_vault']=str(self.root/'vault');config.pop('frozen_data_dir',None)
        for r in (1,2,3):
        ...
        self.assertEqual(result['base_task_executions'],{'min':1980,'max':3060});self.assertEqual(result['external_validation_checkpoints'],['A1','A2','A3'])
        self.assertFalse(config['sequential_domains'])
        self.assertEqual(config['training_tasks_per_pass'],360)
        self.assertEqual(len(result['execution_stages']),3)
        self.assertEqual([s['pre'] for s in result['execution_stages']],[360]*3)
        self.assertEqual([s['domain'] for s in result['execution_stages']],['all']*3)
        self.assertTrue(result['sft_calls'].startswith('0..3'))
        self.assertEqual(result['candidate_limit_per_round'],1)
        self.assertEqual((result['inference_gpus'],result['sft_gpus']),(4,4))
        config['evaluate_initial_system']=True
        self.assertEqual(dry_run(config,self.root)['base_task_executions'],{'min':2280,'max':3360})
