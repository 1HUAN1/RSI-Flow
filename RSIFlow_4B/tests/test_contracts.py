...
TEMP=tempfile.TemporaryDirectory(dir=RUNS)
WORK=Path(TEMP.name)
...
class DataAndEpoch(unittest.TestCase):
    def test_counts_and_separate_configs(self):
        ...
        self.assertEqual(sum(train['train_quotas_per_round'].values())*3,1080)
        self.assertEqual(train['allocated_tasks'],1080)
    ...
    def test_full_manifest_enters_native_prepare_without_resplitting(self):
        ...
        folder=WORK/'data/native_prepare_fixture';folder.mkdir(parents=True,exist_ok=True)
        ...
        expected_seed=initialize_base_manifest(seed)
        ...
        with patch.object(pipeline,'model_identity',return_value={'path':str(folder/'fake_model')}):
            result=pipeline.prepare(cfg)
        self.assertEqual(result['status'],'PREPARED_NOT_TRAINED')
        self.assertEqual(result['harness_identity'],{
            'harness_name':expected_seed.harness_name,'bundle_sha256':expected_seed.bundle_sha256,
            'upstream_commit':expected_seed.upstream_commit})
    ...
    def test_disk_rollout_preserves_scores_and_full_sft_messages(self):
        ...
        root=WORK/'runs/test_override/gen_0/train_rollouts';root.mkdir(parents=True,exist_ok=True)
...
class RoundOrdering(unittest.TestCase):
    def test_three_native_commits_validate_in_order_and_resume(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            class Meta:
                ...
                def diagnose_and_route(self,state,observation,feedback):
                    ...
                    return MetaDecision(action='HARNESS',diagnosis='test_override',evidence=[],rationale='test_override',
                        ...
                        requested_changes=[dict(id='x',component='HARNESS',operation='produce_harness',
                                                target='harness_bundle',instruction='test_override')])
            ...
            fail=[True]
            with patch.object(loop,'build_observation',return_value=observation),patch.object(loop,'updater_capabilities',return_value={}),patch.object(loop,'_accept_meta_update',side_effect=accept),patch('sia.task_meta.round_checkpoint.commit_round_checkpoint',return_value={}):
    ...
    def test_durable_b1_pause_then_same_config_resume_starts_b2(self):
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp);h=root/'h.json';h.write_text('{}')
            task=TaskAgentState(0,'fake',str(h),checkpoint_path='fake')
            meta=MetaAgentState('fake',str(h));save_json(root/'protocol.json',{'config':{}})
            ...
            class Meta:
                ...
                def learn_from_experience(self,state,*args):return {}
            class Protocol:
                total_stages=3;pause_after_round=1;positive_search=False;candidate_limit=1;aligned_interventions=False
                def __init__(self):self.store=SimpleNamespace(mode='parent_pre_update',sequential_domains=False)
                def begin(self,*args):pass
                def bind_execution(self,*args):pass
                def register_rows(self,*args):pass
                def guard(self):pass
                def decision(self,*args):pass
                def check_task_edit(self,*args):pass
                def select_child(self,*args):pass
                def completed(self,*args):pass
                def prepare_meta(self,*args):pass
                def validate_meta(self,*args):pass
                def freeze_system(self,*args):pass
            def accept(root,state,*args):
                state=copy.deepcopy(state);state.version+=1;state.bundle_hash=f'{state.version:064x}';return state
            observation=MetaObservation(0,'T_0',0,{},[],{},[],[],'fake','',[],None,None,None,[],[],
                available_actions={a.value:{'available':a==TaskUpdateAction.HARNESS} for a in TaskUpdateAction})
            def callback(number,record):
                self.assertTrue((root/f'round_{number}/complete.json').exists())
            ...
            updaters={TaskUpdateAction.HARNESS:DurableUpdater(Updater(),StageJournal(root))}
            protocol=Protocol()
            patches=(patch.object(loop,'build_observation',return_value=observation),
                     patch.object(loop,'updater_capabilities',return_value={}),
                     patch.object(loop,'_accept_meta_update',side_effect=accept),
                     patch('sia.task_meta.round_checkpoint.commit_round_checkpoint',return_value={}))
            with patches[0],patches[1],patches[2],patches[3]:
                first=loop.run_sequential_task_meta(root,task,meta,Executor(),Meta(),updaters,
                    max_generations=3,after_round=callback,round_protocol=protocol)
            self.assertEqual((first['status'],first['rounds_completed'],first['paused_at_round']),('paused',1,1))
            self.assertTrue((root/loop.PAUSE_MARKER).is_file())
            self.assertFalse((root/loop.PAUSE_CONSUMED).exists())
            evaluations_before=len([e for e in events if e[0]=='evaluate'])
            patches=(patch.object(loop,'build_observation',return_value=observation),
                     patch.object(loop,'updater_capabilities',return_value={}),
                     patch.object(loop,'_accept_meta_update',side_effect=accept),
                     patch('sia.task_meta.round_checkpoint.commit_round_checkpoint',return_value={}))
            with patches[0],patches[1],patches[2],patches[3]:
                resumed=loop.run_sequential_task_meta(root,task,meta,Executor(),Meta(),updaters,
                    max_generations=3,after_round=callback,round_protocol=protocol,resume=True)
            self.assertEqual((resumed['status'],resumed['rounds_completed']),('completed',3))
            self.assertEqual([e[1] for e in events if e[0]=='validate'],[1,2,3])
            self.assertEqual(len([e for e in events if e[0]=='evaluate'])-evaluations_before,4)
            self.assertTrue((root/loop.PAUSE_CONSUMED).is_file())

    def test_pause_is_not_a_wall_clock_or_generation_budget_substitute(self):
        from sia.task_meta.pipeline import PipelineConfig
        valid=PipelineConfig(training_schedule='round_disjoint',search_dev_fraction=0,max_generations=3,
                             round_protocol={'pause_after_round':1})
        valid.checked()
        with self.assertRaisesRegex(ValueError,'not a max_generations replacement'):
            valid.model_copy(update={'max_generations':1}).checked()
        with self.assertRaisesRegex(ValueError,'only the durable B1 boundary'):
            valid.model_copy(update={'round_protocol':{'pause_after_round':2}}).checked()
        with tempfile.TemporaryDirectory() as tmp:
            budget=Path(tmp)/'budget.json'
            loop.BudgetManager(3,100).persist(budget,resume=False)
            with self.assertRaisesRegex(ValueError,'budget cannot change'):
                loop.BudgetManager(1,100).persist(budget,resume=True)
            with self.assertRaisesRegex(ValueError,'budget cannot change'):
                loop.BudgetManager(3,101).persist(budget,resume=True)
...
class EvaluationReceipts(unittest.TestCase):
    ...
    def test_all_seven_evaluators_run_and_completed_scores_resume(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def predict(p,f,s,out,**kwargs):
                ...
                (out/(name+'.jsonl')).write_text('{"task_id":"fixture"}\n')
            with patch.object(evaluation,'ROOT',root),patch('validation_sources.verify'),patch('sia.task_meta.pipeline.load_config',return_value=SimpleNamespace()),\
                 patch('sia.task_meta.storage.checkpoint_manifest',return_value=[]),\
                 patch('sia.task_meta.harnessforge_manifest.load_manifest',return_value=SimpleNamespace(identity={})),\
                 patch('sia.task_meta.gpu_phases.ensure_services'),patch('sia.task_meta.gpu_phases.verify_services',return_value=[]),patch('evaluation_results.official_task_scores',return_value={}),patch('parallel_predictions.generate_parallel',side_effect=predict),\
    ...
    def test_full_denominator_and_no_nan_or_duplicate_ids(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write(pred,[{'id':'a'},{'id':'b'}]);write(score,[{'accuracy':0.5,'total_count':2,'correct_count':1},{'id':'b','valid':False}])
...
class StartupRecovery(unittest.TestCase):
    def test_failed_host_preflight_can_reenter_native_startup(self):
        ...
        directory=WORK/'runs'/'startup_recovery_fixture'
        attempts=[]
    ...
    def test_train_accepts_verified_paused_result(self):
        import train
        from sia.task_meta import pipeline
        with tempfile.TemporaryDirectory() as tmp:
            root=Path(tmp);marker=root/'pause_after_round.json';marker.write_text('{}')
            config=SimpleNamespace(max_generations=3,round_protocol={'pause_after_round':1})
            result={'status':'paused','rounds_completed':1,'paused_at_round':1,'pause_marker':str(marker)}
            with patch.object(sys,'argv',['train','--config','fixture','--run-dir',str(root)]),\
                 patch.object(pipeline,'load_config',return_value=config),\
                 patch.object(pipeline,'run',return_value=result),patch('builtins.print') as output:
                train.main()
            value=output.call_args.args[0]
            self.assertEqual(value['status'],'paused_at_round_boundary')
            self.assertEqual((value['rounds'],value['resume_starts_at_round']),(1,2))
