...
def run_native(command, work, output, log, handler=None, timeout=604800):
    ...
    changed=False
    # Read the function's top-level literal table, not conditional list
    # transformations. The existing permissions and thread-only filter stay unchanged.
    ...
    evaluator_environment=Path(command[0]).absolute().parent.parent
    if (evaluator_environment/'pyvenv.cfg').is_file():
