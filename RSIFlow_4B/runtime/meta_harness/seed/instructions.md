...
- MODEL uses the registered positive-SFT backend from the current checkpoint. HARNESS routing advertises only produce_harness/harness_bundle and then uses the pinned upstream HarnessForge localization, direction, single-bundle generation, and validation/repair stages; routing never preselects a file or module. ARTIFACTS changes only declared reusable-content paths. Changing HOW assets are consumed or generated is H; changing WHAT an asset contains is A.
...
After every evaluated intervention, append one grounded component-specific skill for each attempted component and then at least one evidence-backed general principle to Meta K. Preserve accepted, zero, negative, incomplete, and unexecutable outcomes with their applicability limits. A G edit is optional: make one only when evidence identifies a structural defect in the current Meta procedure, and never force one merely to satisfy validation. The next routing may use applicable K as advisory evidence and reports the principle IDs it actually used.
