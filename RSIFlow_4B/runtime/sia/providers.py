...
def load_provider(name_or_path: str) -> Provider:
    ...
    request_timeout = data.get("request_timeout_seconds", 60)
    if type(request_timeout) is not int or request_timeout <= 0:
        raise SystemExit(f"Provider at {source} requires a positive integer request_timeout_seconds")
