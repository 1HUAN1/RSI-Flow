...
def training_gpu_lease(gpu, *, minimum_free_mb=40000):
    import fcntl
    configured = os.environ.get('RSIFLOW_OUTPUT_ROOT')
    root = ((Path(configured).resolve() / 'services/resource_leases') if configured else
            Path(__file__).resolve().parents[2] / 'local_baseline/resource_leases')
