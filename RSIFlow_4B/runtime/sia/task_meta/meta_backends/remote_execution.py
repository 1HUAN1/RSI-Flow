...
def _recover_stale_relay(socket_path, directory):
    """Remove only an orphaned Unix socket; never disturb a live relay."""
    try:
        original = socket_path.lstat()
    except FileNotFoundError:
        return
    if not stat.S_ISSOCK(original.st_mode):
        raise BackendUnavailable("META_OPERATION_PENDING", "Relay path exists but is not a Unix socket")
    ...
    try:
        ...
        if exc.errno != errno.ECONNREFUSED:
            raise BackendUnavailable(
                "META_OPERATION_PENDING", "Relay socket ownership could not be proven inactive"
            ) from exc
    else:
        raise BackendUnavailable("META_OPERATION_PENDING", "An active Meta relay already owns the socket")
    finally:
        probe.close()
    ...
    if (not stat.S_ISSOCK(current.st_mode)
        ...
        raise BackendUnavailable("META_OPERATION_PENDING", "Relay path changed during stale-socket recovery")
    socket_path.unlink()
    atomic_json(directory / "remote_relay_recovery.json", {
        "socket": str(socket_path),
        "reason": "stale_socket_connection_refused",
        "recovered_at": time.time(),
        "stale_inode": original.st_ino,
    })
...
def run_remote(backend, prepared, transport):
    ...
    _recover_stale_relay(socket_path, directory)
    ...
    try:
        ...
        if "error" in holder:
            if getattr(holder["error"], "status", None) == "REMOTE_META_REQUEST_REJECTED":
                raise holder["error"]  # Explicit HTTP rejection is not an unknown dispatched job.
            # Keep the transport alive while the original worker completes. Only
            # GET an existing job; never repeat POST /run or provider inference.
            while time.monotonic() - started < remaining and not transport.error:
                try:
                    existing = rpc(config, "GET", "/jobs/" + prepared.request.request_id,
                                   timeout=min(10, remaining - (time.monotonic() - started)), maximum=maximum)
                    ...
                    pass
                time.sleep(min(2, max(0, remaining - (time.monotonic() - started))))
            if "result" not in holder:
                raise BackendUnavailable("META_OPERATION_PENDING", "Original SSH job remains unresolved; no inference resubmitted") from holder["error"]
