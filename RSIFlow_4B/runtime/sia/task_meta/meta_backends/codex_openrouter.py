...
from sia.task_meta.prompts import RUNTIME_RELIABILITY_CONTRACT
...
def recovery_for_operation(path, context, operation, decision_id, bundle_hash):
    """An interrupted budget belongs to one decision, not every route in a round."""
    if not path.is_file():
        return None
    recovery = json.loads(path.read_text())
    if operation != recovery.get('operation', 'routing') or context['generation'] != recovery.get('generation', 3):
        return None
    source = Path(recovery['source_budget'])
    if sha256(source.read_bytes()) != recovery['source_budget_sha256']:
        raise ValueError('Interrupted source budget changed')
    binding = json.loads((source.parent / 'operation.json').read_text())
    if (binding['context'] != context or binding['operation'] != operation
            or binding['decision_id'] != decision_id or binding['meta_harness_hash'] != bundle_hash):
        return None
    return recovery


def invalid_candidate_for_repair(prepared, raw_output):
    ...
    if candidate_path.is_file() and not candidate_path.is_symlink():
        try:
            envelope = json.loads(candidate_path.read_text())
            identity = prepared.request.identity()
            if (isinstance(envelope, dict) and set(envelope) == {*identity, "result"}
                    and all(type(envelope.get(k)) is type(v) and envelope.get(k) == v for k, v in identity.items())
                    and isinstance(envelope["result"], dict)):
                return envelope["result"]
        except (ValueError, OSError):
            pass
    try:
        return json.loads(raw_output)["result"]
...
def render_codex_config(config: MetaBackendConfig, *, isolated=False):
    ...
    lines = [f"model = {json.dumps(config.model)}", f"model_provider = {json.dumps(config.provider)}",
             ...
             'web_search = "disabled"']
    lines += ['model_reasoning_effort = "low"']
    if isolated:
...
class CodexOpenRouterBackend:
    ...
    def prepare(self, prompt, schema, *, meta_state=None, operation=None, decision_id=None,
        ...
        bound_files = bundle.read_files()
        if "principles.json" in bound_files:
            evidence_files["meta_input/self_update_protocol.md"] = bound_files["self_update_protocol.md"]
            evidence_files["meta_input/principles.json"] = bound_files["principles.json"]
        ...
        if self._candidate_file_operation(operation):
            ...
            contract = (
                ...
                "Execution environment: use python3; the python command is unavailable. Read the schema, build the actual object, write all contents (no placeholders, labels or ellipses), "
        ...
        # Protected response-contract section is retained even when G crops evidence.
        # Apply to routing, H/A/M proposals, repairs and G self-updates alike.
        contract = (RUNTIME_RELIABILITY_CONTRACT + "\nCurrent Meta operation ceilings "
                    "(not Task context limits or remaining account credit):\n"
                    + json.dumps(request.budget.model_dump(), sort_keys=True)
                    + "\n\n" + contract)
        ...
        if "principles.json" in bound_files:
            from sia.task_meta.meta_harness.five_stage import identity, library_views
            atomic_json(directory / "principle_load.json", {**identity(bound_files),
                "g_hash": bundle.hash, "operation": operation, "decision_id": decision_id,
                "experience_id": experience_id, "views": library_views(json.loads(bound_files["principles.json"])),
    ...
    def validate(self, prepared):
        ...
        endpoints = {
            "openrouter": ("https://openrouter.ai/api/v1", "OPENROUTER_API_KEY", {
                "deepseek/deepseek-v4-flash-0731", "deepseek/deepseek-v4.1-flash", "openai/gpt-5.6-sol:batch", "openai/gpt-5.6-sol"}),
            "autodl": ("https://www.autodl.art/api/v1", "AUTODL_API_KEY", {"DeepSeek-V4.1-Flash", "DeepSeek-V4-Flash", "DeepSeek-V4-Flash-0731", "gpt-5.6-sol", "gpt-6-astra"}),
        }
        ...
        elif c.execution_location == "local_chroot":
            from .local_execution import validate_local
            atomic_json(prepared.directory / "execution_host.json", validate_local(c))
        else:
            self._bubblewrap()
    ...
    def compatibility_identity(self):
        ...
        return {"backend": c.backend, "provider": c.provider, "base_url": c.base_url, "model": c.model, "codex_commit": c.codex_commit,
                ...
                **({"local_execution": __import__(
                    "sia.task_meta.meta_backends.local_execution", fromlist=["runtime_identity"]).runtime_identity(c)}
                   if c.execution_location == "local_chroot" else {})}
    ...
    def run(self, prepared):
        ...
        try:
            ...
            if self.config.execution_location == "local_chroot":
                from .local_execution import run_local
                return run_local(self, prepared, transport)
    ...
    def collect(self, prepared, result):
        ...
        from .transport import valid_attempt_chain
        if result.get("transport_error") or not valid_attempt_chain(result.get("transport", []), self.config.accepted_response_models):
    ...
    def _complete_evolved(self, schema, *, meta_state, operation, decision_id, experience_id,
        ...
        recovery_path = self.journal / 'interrupted_routing_recovery.json'
        recovery = recovery_for_operation(recovery_path, self.context, operation, decision_id, bundle.hash)
        if recovery is not None:
            operation_input['instruction'] = operation_input.get('instruction', '') + '\n\nDEPLOYMENT RECOVERY (not an accepted decision): ' + json.dumps(recovery['evidence'], ensure_ascii=False)
        ...
        with exclusive_lock(directory / ".lock"):
            ...
            carry_path = self.journal / "rejected_routing_carry.json"
            if operation == "routing" and self.context["generation"] == 0 and carry_path.exists() and not (directory / "budget.json").exists():
                carry = json.loads(carry_path.read_text())
                source = Path(carry["source_budget"])
                if sha256(source.read_bytes()) != carry["source_budget_sha256"]:
                    raise ValueError("Original charged budget changed")
                transport = Path(carry["source_transport"])
                if sha256(transport.read_bytes()) != carry["source_transport_sha256"]:
                    raise ValueError("Original rejection evidence changed")
                snapshot = Path(carry["authorized_budget_snapshot"])
                if sha256(snapshot.read_bytes()) != carry["authorized_budget_sha256"]:
                    raise ValueError("Authorized budget snapshot changed")
                prior = json.loads(snapshot.read_text())
                if prior["limits"] != self.config.budget.model_dump() or prior["pending_requests"] != 0:
                    raise ValueError("Authorized budget identity mismatch")
                original = json.loads(source.read_text())
                if (prior["requests"] != original["requests"] or prior["output_tokens"] != original["output_tokens"]
                        or prior["unresolved_output_token_reserve"] != original.get("unresolved_output_token_reserve", 0) + original["pending_requests"] * original["limits"]["max_output_tokens_per_request"]):
                    raise ValueError("Prior usage or unknown reservation was not retained")
                atomic_json(directory / "budget.json", prior)
            resumed = self.journal / "resumed_meta_budget.json"
            if resumed.exists() and operation == 'meta_self_update' and self.context['generation'] == json.loads(resumed.read_text()).get('generation', 3) and not (directory / 'budget.json').exists():
                carry = json.loads(resumed.read_text())
                source = Path(carry['source_budget'])
                if sha256(source.read_bytes()) != carry['source_sha256']:
                    raise ValueError('Resumed operation source ledger changed')
                before = json.loads(source.read_text()); after = carry['snapshot']
                released = 0
                allowed = {entry['evidence']: entry['reserved_output_tokens'] for entry in before.get('terminal_failures', [])}
                seen = set()
                for entry in carry.get('reconciled_http_rejections', []):
                    path = entry['evidence']
                    if path in seen or path not in allowed or sha256(Path(path).read_bytes()) != entry['sha256']:
                        raise ValueError('Unbound or changed HTTP rejection evidence')
                    record = json.loads(Path(path).read_text())
                    if record.get('http_status') != 429 or record.get('completed') or record.get('terminal_failure', {}).get('code') != 'http_429':
                        raise ValueError('Only explicit pre-response HTTP429 reservations can be released')
                    seen.add(path)
                    released += allowed[path]
                if (after['limits'] != self.config.budget.model_dump() or after['requests'] != before['requests']
                        or after['output_tokens'] != before['output_tokens'] or after['pending_requests'] != 0
                        or after['unresolved_output_token_reserve'] != before.get('unresolved_output_token_reserve', 0) + before['pending_requests'] * self.config.budget.max_output_tokens_per_request - released):
                    raise ValueError('Resumed operation accounting mismatch')
                atomic_json(directory / 'budget.json', after)
            if recovery and not (directory / 'budget.json').exists():
                before = json.loads(Path(recovery['source_budget']).read_text())
                after = recovery['budget_snapshot']
                if (after['limits'] != self.config.budget.model_dump() or after['requests'] != before['requests']
                    or after['output_tokens'] != before['output_tokens'] or after['started_at'] != before['started_at']
                    or after['pending_requests'] != 0 or after['network_bytes'] != before['network_bytes']
                    or after['unresolved_output_token_reserve'] != before.get('unresolved_output_token_reserve', 0) + before['pending_requests'] * self.config.budget.max_output_tokens_per_request):
                    raise ValueError('Interrupted request usage/reservation changed')
                atomic_json(directory / 'budget.json', after)
            ...
            def invoke_stage(stage_id, stage_prompt, stage_schema, evidence_files=None, allowed_paths=None):
                ...
                if receipt:
                    ...
                    if receipt["state"] != "prepared":
                        ...
                        call_status = json.loads(status_path.read_text()) if status_path.exists() else {}
                        undispatched_remote_stage = (
                            receipt["state"] == "pending"
                            and self.config.execution_location == "ssh_worker"
                ...
                try:
                    ...
                    invalid_output = invalid_candidate_for_repair(prepared, raw_output)
                    exc.repair_candidate = invalid_output
            ...
            try:
                output = execute(bundle, operation, operation_input, schema, invoke_stage, directory / "harness_runtime",
                                 external_budget={"max_invocations": min(64, self.config.budget.max_requests),
