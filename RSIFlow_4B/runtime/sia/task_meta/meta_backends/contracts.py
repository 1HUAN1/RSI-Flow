...
class MetaBudget(BaseModel):
    ...
    wall_time_seconds: int = Field(default=300, ge=1, le=7200)
    max_requests: int = Field(default=12, ge=1, le=128)
    ...
    max_processes: int = Field(default=256, ge=4, le=512)
...
class MetaBackendConfig(BaseModel):
    ...
    execution_location: Literal["local", "ssh_worker", "local_chroot"] = "local"
    ...
    def expected_response_model(self):
        ...
        if (self.provider, self.model) == ("autodl", "DeepSeek-V4.1-Flash"):
            # Observed AutoDL Responses identity; the request remains version-pinned.
