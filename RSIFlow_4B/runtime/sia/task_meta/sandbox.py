...
def _restrict_filesystem(read_paths: list[Path], scratch: Path, *, write_paths: tuple[Path, ...] = ()) -> None:
    libc = ctypes.CDLL(None, use_errno=True)
    ...
    try:
        for path, access in [(p, read_access) for p in read_paths] + [(scratch, handled)] + [(p, handled) for p in write_paths]:
...
def _restrict_syscalls(library: str = "libseccomp.so.2", *, allow_threads: bool = False) -> None:
    ...
    if allow_threads:
        denied = [name for name in denied if name not in {"clone", "clone3"}]
        # Disposable validation workers may create threads, but descendants must
        # remain in the worker process group for timeout/exit cleanup.
        denied.extend(["setsid", "setpgid", "init_module", "finit_module", "delete_module"])
    try:
        for name in denied:
