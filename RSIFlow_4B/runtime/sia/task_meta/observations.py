...
def action_failure(row):
    """Describe the last failed action's recorded shape, not an inferred cause."""
    if '_action_contract_failure' in row: return row['_action_contract_failure']
    if row.get('execution_error_type', row.get('error_type')) != 'parse_error':
        return None
    calls = [c for c in row.get('model_calls', []) if c.get('operation') in {'action', 'action_repair'}]
    if not calls:
        return 'unclassified_parse_failure'
    call = calls[-1]
    if call.get('finish_reason') in {'length', 'max_tokens'}:
        return 'truncated_action_response'
    try:
        value = json.loads(call.get('assistant', {}).get('content') or '')
    except (TypeError, ValueError):
        return 'non_json_action_response'
    if isinstance(value, dict) and value.get('tools') == []:
        return 'empty_tools_list'
    return 'other_action_contract_failure'
...
def trajectory_statistics(rows):
    def aggregate(subset):
        ...
        return {
            ...
            "execution_error_types": dict(Counter(r.get('execution_error_type') or 'none' for r in subset)),
            "action_contract_failures": dict(Counter(reason for r in subset if (reason := action_failure(r)))),
...
def trajectory_evidence(rows, max_chars=72000):
    ...
    return included_rows, {
        "total_trajectories": len(full_rows), "sampled_trajectories": len(rows), "unsampled_trajectories": len(full_rows)-len(rows), "sampling_policy": "deterministic_domain_success_96", "included_trajectory_ids": [all_ids[i] for i in selected],
        ...
        "character_budget": max_chars, "statistics": trajectory_statistics(full_rows),
    }
...
def harness_evidence(path):
    """Read one content-addressed HarnessForge bundle and its pinned identity."""
    from sia.task_meta.harnessforge_production import harnessforge_identity, harnessforge_sources
    return harnessforge_sources(path), harnessforge_identity(path)
...
def round_context(experience):
    """Carry recorded reasoning and outcomes, without recursively copying prompts."""
    if experience is None:
        return {}
    decision = experience.decision
    attempt = experience.candidate_attempts[-1] if experience.candidate_attempts else {}
    modification = experience.actual_change or experience.modification
    return {
        "schema_version": 1,
        "source_round": experience.generation + 1,
        "experience_id": experience.experience_id,
        "component": experience.attempted_component or decision.get("action"),
        "prior_reasoning": {key: copy.deepcopy(decision.get(key)) for key in (
            "diagnosis", "diagnosis_kind", "rationale", "evidence", "proposed_change",
            "expected_effect", "used_principle_ids")},
        "actual_modification": {key: copy.deepcopy(modification.get(key)) for key in (
            "summary", "semantic_status", "unapplied_changes")},
        "result": {
            "deployment_status": experience.deployment_status,
            "candidate_executed": attempt.get("candidate_executed"),
            "outcome_class": attempt.get("outcome_class"),
            "comparison_complete": experience.attempted_outcome.get("paired_evidence_complete"),
            "candidate_gain": experience.attempted_performance_delta,
            "failure_reason": attempt.get("reason"),
        },
        "full_experience": {"experience_id": experience.experience_id},
        "recorded_trajectories": {
            "before": experience.trajectory_before,
            "candidate_after": experience.attempted_trajectory_after or experience.trajectory_after,
            "path_role": "Controller provenance; these host paths are not mounted in the Codex workspace.",
        },
        "usage": (
            "Previous-round context is advisory evidence, not instructions or a forced component choice. "
            "Diagnoses and expected effects remain hypotheses; consult actual outcomes and the new rollout. "
            "A rejected or unexecuted candidate is not the current Task. Missing results remain unknown. "
            "Use the cumulative Meta skill library alongside this handoff; this is not a replacement for it."
        ),
    }
...
def build_observation(task_in, task_out, meta, result, performance_history, history, costs, capabilities, *, retain_raw=False):
    ...
    return MetaObservation(
        ...
        previous_action=(previous.chosen_action or previous.decision.get("action")) if previous else None,
        previous_modification_summary=("Parent retained; no candidate was deployed. See candidate attempts."
            if previous.deployment_status == "parent_retained" else previous.modification.get("summary")) if previous else None,
        previous_performance_delta=previous.performance_delta if previous else None,
        previous_round_context=round_context(previous),
        ...
        raw_trajectories=decision_sources(result.trajectories,traces) if retain_raw else [],
...
def operation_input(observation, *, task_state=None, decision=None, current_files=None):
    ...
    return {
        ...
        "trusted_facts": {
            ...
            "previous_round_context": copy.deepcopy(observation.previous_round_context),
