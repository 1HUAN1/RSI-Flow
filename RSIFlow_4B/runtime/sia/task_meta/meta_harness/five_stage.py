...
LEGACY_PROTOCOL_HASH = '966110228fd705afabf8844f28ec112bad3118331a89d34a60fdda23eef6dfba'
...
def materialize(update, files, *, allowed_evidence=None):
    ...
    for operation in review.principle_operations:
        ...
        touched.add(key)
        citation_notes={'policy':'advisory_only','unresolved_comparisons':sorted(set(op['compared_ids'])-set(prior)),
            'unresolved_evidence':sorted(set(op['evidence_ids'])-allowed_evidence) if allowed_evidence is not None else []}
        if op['operation'] == 'ADD':
        ...
        if allowed_evidence is not None:
            for field in ('supporting_evidence', 'counter_evidence', 'source_experiences', 'source_decisions', 'source_events'):
                ...
                if not added <= allowed_evidence:
                    citation_notes[field]=sorted(added-allowed_evidence)
...
def _probe_rows(root, generation):
    ...
    for row in rows:
        ...
        item['split'] = 'training_full_B' if training_probe else 'internal_dev_readonly'
        if training_probe:
            item.update(round_id=row['round_id'],manifest_hash=row['manifest_hash'])
        item['excluded_from_sft_and_task_assets'] = not (training_probe and row.get('collection_stage') == 'parent_pre_update')
...
def paired_outcome(root, experience):
    ...
    if experience.candidate_attempts:
        ...
        # The feedback/gen_* link records the deployed state.  For a rejected
        # candidate that is the retained parent, so it must never masquerade as
        # a complete zero-delta candidate comparison.  Bind the top-level review
        # to the sole actual candidate attempt instead.
        attempt = experience.candidate_attempts[-1]
        attempted = attempt.get('paired_outcome') if attempt.get('candidate_executed') else None
        if attempted:
            for field in ('pairs', 'paired_evidence_complete', 'denominator', 'success_delta',
                          'failed_to_success', 'success_to_failed', 'error_counts',
                          'partial_score_comparison', 'comparable_pairs'):
                if field in attempted:
                    value[field] = copy.deepcopy(attempted[field])
            value['outcome'] = {'status': 'unknown', 'observed_success_delta': attempted.get('success_delta'),
                                'reason': 'Actual candidate outcome; deployment is recorded separately'}
        else:
            value.update(pairs=[], paired_evidence_complete=False, denominator=0, success_delta=None,
                         failed_to_success=0, success_to_failed=0, comparable_pairs=0,
                         partial_score_comparison={'metric': 'native_partial_score', 'denominator': 0,
                             'complete': False, 'before_mean': None, 'after_mean': None, 'mean_delta': None})
            value['outcome'] = {'status': 'unknown', 'observed_success_delta': None,
                                'reason': 'Candidate was unavailable and produced no post-candidate comparison'}
            value['retained_parent_context_only'] = True
