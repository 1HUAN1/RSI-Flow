...
def execute(bundle, operation, operation_input, schema, invoke_stage, audit_dir, *,
    ...
    def review_source_ids():
        ...
        extra=envelope.get('trusted_facts',{}).get('recursive_effects',{})
        allowed.update(x['source_id'] for x in extra.get('representatives',[]))
        if extra.get('source_id'): allowed.add(extra['source_id'])
        boundary=envelope.get('trusted_facts',{}).get('outer_round_consolidation')
        if boundary:
            allowed.add(boundary['source_id'])
            allowed.update(x['experience_id'] for x in boundary['evidence'])
            ...
            try:
                ...
                if five_identity:
                    ...
                    if operation == "routing":
                        if typed.expectations is None:
                            errors.append("Routing needs expectations")
                        audit["unresolved_principle_citations"]=sorted(set(typed.used_principle_ids + (typed.expectations.principle_ids if typed.expectations else []))-active_ids)
                    ...
                    if operation in {"meta_self_update", "final_consolidation"}:
                        ...
                        for finding in (typed.five_stage.implementation, typed.five_stage.activation, typed.five_stage.outcome):
                            ...
                            if unavailable:
                                audit.setdefault("unresolved_review_citations",[]).extend(sorted(unavailable))
    ...
    def model_step(step, stage_id, *, repair=False):
        ...
        if five_identity:
            ...
            if memory_contract and memory_contract.get("allow_mechanism_revision"):
                payload["principle_contract"]["targets"] = "New ADD records may bind changed Meta instructions/nodes. Preserve previous K bytes; use native slow_edit only with recorded residual mismatches. Task state remains frozen."
        ...
        prior_context = envelope.get("trusted_facts", {}).get("previous_round_context")
        if prior_context and envelope.get("experiences"):
            stage_files["meta_input/previous_round_experience.json"] = _json(envelope["experiences"][-1])
            payload["previous_round_context_file"] = "meta_input/previous_round_experience.json"
            stage_files["meta_input/operation.json"] = _json(payload)
