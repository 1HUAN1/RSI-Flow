...
DOMAIN_ORDER = ('tool_use', 'code', 'searchqa')


def stage_manifest(root, stage_id, *, sequential_domains=False):
    """A derived execution view; never changes the original three frozen B manifests.

    Native round_id remains the monotonic update/stage ID used by recovery and
    evidence guards. allocation_round_id records the user's outer round (1..3).
    """
    maximum = 9 if sequential_domains else 3
    if type(stage_id) is not int or not 1 <= stage_id <= maximum:
        raise ValueError('Execution stage outside the frozen schedule')
    outer = (stage_id-1)//3+1 if sequential_domains else stage_id
    parent = manifest(Path(root)/f'B{outer}/manifest.json', 'train_evolution', outer)
    if not sequential_domains: return parent
    from collections import Counter
    if Counter(t['source'] for t in parent['tasks']) != TRAIN_QUOTAS:
        raise ValueError('Domain schedule requires the unchanged 360-task allocation')
    domain = DOMAIN_ORDER[(stage_id-1)%3]
    tasks = [{**t, 'round_id':stage_id, 'allocation_round_id':outer} for t in parent['tasks'] if t['domain']==domain]
    if len(tasks)!=120: raise ValueError('Domain stage must contain exactly 120 tasks')
    return make_manifest('train_evolution', stage_id, tasks, allocation_round_id=outer,
        execution_stage_id=stage_id, domain=domain, allocation_manifest_hash=parent['manifest_hash'])
...
def training_provenance(rows, current_manifest, *, round_id, branch_id, harness_hash, parent_policy_hash):
    ...
    for row in rows:
        ...
        if row.get('derived_from') or row.get('meta_generated') or row.get('submission_origin'):
...
def success_filter(rows, *, minimum_samples, native_selector):
    ...
    reasons = Counter(); candidates = []; decisions = []; seen_trajectories = set()
    for row in rows:
        item = dict(task_id=row.get('task_id'), trajectory_id=row.get('trajectory_id'),
                    behavior_harness=row.get('harness_hash'), round_id=row.get('round_id'))
        identity = row.get('trajectory_id') or fingerprint(row)
        if identity in seen_trajectories:
            reasons['duplicate_supervision'] += 1
            decisions.append({**item, 'reason':'duplicate_trajectory', 'kept':False}); continue
        seen_trajectories.add(identity)
        verification = row.get('verification', {})
        if row.get('infrastructure_error') or row.get('submission_origin') or row.get('meta_generated') or verification.get('status')!='completed' or verification.get('success') is not True:
            reasons['not_verified_success'] += 1
            decisions.append({**item, 'reason':'not_verified_success', 'kept':False})
        else:
            ...
            decisions.append({**item, 'reason':'verified_success' if samples else 'native_supervision_rejected',
                              'kept':bool(samples), 'decision_samples':len(samples)})
    from_seen = {}; selected = []
    ...
    report['trajectory_decisions'] = decisions
...
def meta_routing_rows(rows, *, maximum=48, per_domain=16, per_outcome=8, seed=42):
    """Select detailed Meta evidence with fixed domain/outcome coverage.

    Every source row still contributes its score/provenance summary.  This
    selector controls only which rows receive bounded event/tool/model-call
    excerpts in the routing context.
    """
    domains=DOMAINS
    if maximum != len(domains)*per_domain or per_domain != 2*per_outcome:
        raise ValueError('Meta routing quotas must be 3x16 with 8 success and 8 failure')
    unknown=sorted({row.get('domain') for row in rows}-set(domains),key=str)
    if unknown:
        raise ValueError('Unknown Meta routing domains: '+', '.join(map(str,unknown)))

    def rank(row, outcome):
        return fingerprint([seed,row.get('domain'),outcome,row.get('task_id'),
                            row.get('rollout_id'),row.get('trajectory_id')])

    def ranked_failures(values):
        groups=defaultdict(list)
        for row in values:
            error=(row.get('execution_error_type') or row.get('error_type')
                   or 'verification_failure')
            groups[str(error)].append(row)
        for error in groups:
            groups[error].sort(key=lambda row:rank(row,'failure'))
        ordered=[]
        index=0
        while True:
            progressed=False
            for error in sorted(groups):
                if index < len(groups[error]):
                    ordered.append(groups[error][index]);progressed=True
            if not progressed:
                return ordered
            index+=1

    def remaining_order(successes, failures):
        # Extra cross-domain capacity stays outcome-balanced where possible.
        ordered=[];s=f=0
        while s<len(successes) or f<len(failures):
            if s<len(successes):
                ordered.append(successes[s]);s+=1
            if f<len(failures):
                ordered.append(failures[f]);f+=1
        return ordered

    selected=[];extras={}
    for domain in domains:
        values=[row for row in rows if row.get('domain')==domain]
        successes=sorted(
            [row for row in values if row.get('verification',{}).get('success') is True],
            key=lambda row:rank(row,'success'))
        failures=ranked_failures(
            [row for row in values if row.get('verification',{}).get('success') is not True])
        chosen_success=successes[:per_outcome]
        chosen_failure=failures[:per_outcome]
        success_rest=successes[len(chosen_success):]
        failure_rest=failures[len(chosen_failure):]
        room=per_domain-len(chosen_success)-len(chosen_failure)
        # A short outcome gives all of its unused quota to the other outcome.
        if room and len(chosen_success)<per_outcome:
            take=min(room,len(failure_rest));chosen_failure+=failure_rest[:take]
            failure_rest=failure_rest[take:];room-=take
        if room and len(chosen_failure)<per_outcome:
            take=min(room,len(success_rest));chosen_success+=success_rest[:take]
            success_rest=success_rest[take:];room-=take
        selected.extend(chosen_success+chosen_failure)
        extras[domain]=remaining_order(success_rest,failure_rest)

    # If one domain has fewer than 16 rows, deterministically round-robin the
    # still-available capacity of the other domains until 48 or exhaustion.
    offsets=dict.fromkeys(domains,0)
    while len(selected)<maximum:
        progressed=False
        for domain in domains:
            index=offsets[domain]
            if index<len(extras[domain]) and len(selected)<maximum:
                selected.append(extras[domain][index]);offsets[domain]+=1;progressed=True
        if not progressed:
            break
    return selected
