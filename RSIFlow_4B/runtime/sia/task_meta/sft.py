...
class SampleLengthExceeded(ValueError):
    def __init__(self, token_count, max_length):
        self.token_count = token_count
        super().__init__(f"Positive trajectory has {token_count} tokens, exceeding --max-length={max_length}")


def select_length_eligible(tokenizer, rows, max_length, *, supervision):
    """Exclude whole overlength calls; never truncate context or supervision.

    Other encoding errors still fail closed. These are independent recorded
    calls from successful training trajectories, not fragments of failed tasks.
    """
    selected, lengths, excluded = [], [], []
    for index, row in enumerate(rows):
        try:
            encoded = encode_messages(tokenizer, row['messages'], max_length,
                supervision=supervision, tools=row.get('tools'),
                chat_template_kwargs=row.get('chat_template_kwargs'))
        except SampleLengthExceeded as exc:
    ...
    return selected, {'policy': 'whole_recorded_call_length_filter_v1', 'max_length': max_length,
        'input_samples': len(rows), 'eligible_samples': len(selected),
        'eligible_token_lengths': lengths, 'excluded': excluded, 'truncation_applied': False}
...
def validate_training_row(row, profile="legacy_gpqa"):
    ...
    if row.get('infrastructure_error'):
        raise ValueError('Infrastructure failure is not verified SFT success')
    if row.get('source_role')=='train_evolution' and row.get('collection_stage')!='parent_pre_update':
        raise ValueError('Only parent pre is eligible for SFT')
    if row.get('submission_origin'):
        raise ValueError('Direct submission evaluations are not fresh Task-model SFT trajectories')
...
def encode_messages(tokenizer, messages, max_length, *, supervision="final_assistant", tools=None,
    ...
    if len(full) > max_length:
        raise SampleLengthExceeded(len(full), max_length)
    labels = [-100] * len(full)
