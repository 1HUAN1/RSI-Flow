...
RUNS_ROOT = Path(os.environ.get(
    "TASK_META_RUNS_ROOT", Path(__file__).resolve().parents[2] / "runs")).resolve()
...
def register_checkpoint(request: CheckpointRequest):
    ...
    if (not checkpoint.is_relative_to(RUNS_ROOT.resolve()) or not checkpoint.is_dir()
        ...
        raise HTTPException(400, "Checkpoint ID must equal its absolute directory inside output_root/runs")
    with lock:
