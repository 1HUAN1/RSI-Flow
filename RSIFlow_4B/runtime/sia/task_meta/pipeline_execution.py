...
class MultiDomainExecutor:
    ...
    def _one(self, state, harness_spec, task, rollout, directory, artifacts_text, *, probe, artifact_sources=None):
        ...
        try:
            ...
            memory_namespace = 'serial'
            if self.replicas:
                endpoint = str(getattr(self, '_replica_endpoint', 'unbound'))
                memory_namespace = 'replica_' + hashlib.sha256(endpoint.encode()).hexdigest()[:16]
            memory_root = (Path(getattr(self, '_harnessforge_memory_root',
                                directory.parent / 'harnessforge_memory')) / memory_namespace)
            result = run_seed(harness_spec, model, environment, json.dumps(public, ensure_ascii=False),
                              artifacts_text, random_seed, artifact_sources=artifact_sources,
                              memory_storage_root=memory_root,
                              bench_type=task.domain,
                              max_model_calls=self.model_call_limit,
                              max_tool_calls=self.model_call_limit,
                              max_tokens=self.max_output_tokens)
            score = result.pop('_evaluation', None)
    ...
    def execute(self, state, directory):
        ...
        harness_path = Path(state.harness_path)
        harness_spec = load_manifest(harness_path)
        ...
        memory_owner = (
            Path(self.round_protocol.root) if getattr(self, "round_protocol", None) else next(
                (parent for parent in (directory, *directory.parents)
                 if (parent / 'budget.json').is_file()),
                directory.parent,
            )
        )
        self._harnessforge_memory_root = str(
            (memory_owner / 'harnessforge_memory' / harness_spec.bundle_sha256).resolve())
        artifact_sources = []
        ...
        train = self._batch(frozen, harness_spec, ((t, n) for t in tasks for n in range(self.rollouts_per_task)),
        ...
        probe = train if getattr(self,'round_protocol',None) else self._batch(frozen, harness_spec, [(t, n) for t in self.store.probe() for n in range(self.probe_rollouts)],
        ...
        costs = {'wall_time_seconds': sum(r['wall_time_seconds'] for r in (train if getattr(self,'round_protocol',None) else train + probe)),
                 'model_calls': sum(r['model_call_count'] for r in (train if getattr(self,'round_protocol',None) else train + probe)),
                 'input_tokens': sum(r['input_tokens'] for r in (train if getattr(self,'round_protocol',None) else train + probe)),
                 'output_tokens': sum(r['output_tokens'] for r in (train if getattr(self,'round_protocol',None) else train + probe)),
                 'unknown_usage_calls': sum(r.get('unknown_usage_calls', 0) for r in (train if getattr(self,'round_protocol',None) else train + probe)),
                 'usage_complete': all(r.get('usage_complete', False) for r in (train if getattr(self,'round_protocol',None) else train + probe)),
